...
# 打開你想要填寫的網頁表單
driver.get(
    'https://qfreeaccountssjc1.az1.qualtrics.com/jfe/form/SV_7VA5FzxUFVOmfae')
...
question_blocks = driver.find_elements(
    By.CSS_SELECTOR, 'div.QuestionOuter.BorderColor')

# 迭代每個問題區塊
for question_block in question_blocks:
    # 找到問題區塊中的 ul.ChoiceStructure 元素
    choice_structure_element = question_block.find_element(
        By.CSS_SELECTOR, 'ul.ChoiceStructure')

    # 找到 ul.ChoiceStructure 元素下的所有 li.Selection.reg 元素
    selection_elements = choice_structure_element.find_elements(
        By.CSS_SELECTOR, 'li.Selection.reg')
    ...
    if len(selection_elements) > 0:
        ...
        input_element = selected_element.find_element(
...
while True:
    ...
    for question_row in question_rows:
        # 找到題目中的所有選項
        td_elements = question_row.find_elements(
            By.CSS_SELECTOR, 'td[class^="c"]')
        if len(td_elements) > 0:
            # 隨機選擇一個選項
            selected_td = random.choice(td_elements)

            # 找到選擇的 td 元素中的 <input> 元素
            input_element = selected_td.find_element(
